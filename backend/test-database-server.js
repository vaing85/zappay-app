...
console.log('🧪 Testing ZapPay Production Server v2 with Database...');
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode === 200) {
          console.log(`   ✅ SUCCESS`);
        } else if (res.statusCode === 503 && path === '/api/db-test') {
          console.log(`   ⚠️  EXPECTED (Database not connected)`);
        } else {
          console.log(`   ❌ FAILED`);
        }
        
        resolve(res.statusCode === 200 || (res.statusCode === 503 && path === '/api/db-test'));
      });
    });
...
async function runTests() {
  ...
  if (passed === total) {
    console.log(`\n🎉 ALL TESTS PASSED! Database server is working perfectly!`);
  } else {
